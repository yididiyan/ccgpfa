...
def load_data(filename, trial_index=0):
  import pickle 
  import glob 
  ...
  # train test split 
  return latents, Y, None, non_zero_indices 
...
latents, y_train, y_test, non_zero_indices = load_data(args.dataset, trial_index=args.trial)
##params##
...
plt.savefig('maze_results_'+ str(args.trial) + '.png')
plt.show()
...
padded_rate = zero_pad((len(non_zero_indices), N),  rate, non_zero_indices)
...
result = {
        'train_loglik_mean': train_loglik.sum(axis=(1,2)).mean(),
        'train_loglik_std': train_loglik.sum(axis=(1,2)).std(),
        ...
        'train': {
          'firing_rates': padded_rate,
        }
    }

from pathlib import Path 

fname = Path(args.dataset).stem

os.makedirs('maze_results.final', exist_ok=True)

with open('maze_results.final/' + fname + str(args.trial) + '_' + str(result['train_loglik_mean']) + '_seed_' + str(args.seed) + '.pkl', 'wb') as f_:
    print(result['train_loglik_mean'], result['train_loglik_std'])
