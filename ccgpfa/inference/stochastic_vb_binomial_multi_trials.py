...
from ccgpfa.inference.vb_binomial_multi_trial import VariationalBayesInference, generate

class StochasticSparseVariationalInference(VariationalBayesInference):


    def __init__(self, Y, D=10, M=5, lr=5e-2, device=None, ell_init=8., ngd_lr=0.6, batch_size=10, **kwargs):
    ...
    def B_(self):
        if self.tied_trials:
            ...
            # return self.M * self.n[..., None] + self.Y_sum[..., self.sample_indices]
        # return [self.n[..., None] + self.Y[m][..., self.sample_indices] for m in range(self.M)]
        return [self.n[..., None].expand(self.N, len(self.sample_indices)) for _ in range(self.M)]
    ...
    def Kappa(self):
        # return 0.5 * (self.Y_sum[..., self.sample_indices] - self.M * self.n[..., None])
        return (self.Y_sum[..., self.sample_indices] - 0.5 * self.M * self.n[..., None])
